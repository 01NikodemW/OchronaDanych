import { Alert, Box, Button, Checkbox, FormControlLabel, FormGroup, List, ListItem, Modal, TextField, Typography } from '@mui/material'
...
export default function MainPage() {
    ...
    const [isInsertPasswordModalOpen, setIsInsertPasswordModalOpen] = useState<boolean>(false)
    const [notePasswordToDecrypt, setNotePasswordToDecrypt] = useState<string>("")
    ...
    async function changeAccessibility(noteId: string) {
        ...
        const response = await fetch(
            "https://localhost:7154/api/notes",
            {
                method: "PUT",
                mode: 'cors',
                headers: {
    ...
    async function getMyNotes() {
        ...
        const response = await fetch(
            "https://localhost:7154/api/notes/mynotes",
            {
                method: "GET",
                mode: 'cors',
                headers: {
        ...
        <>
            ...
            <Box sx={{
                ...
                <Box sx={{
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", height: "100px", alignItems: "center" }}>
                    ...
                    {showCreateNote &&
                        <Box>
                            <Box sx={{ display: "flex", justifyContent: "space-around", paddingBottom: "20px" }}>
                                <Button
                                    sx={{
                                        bgcolor: "#E9E9EB",
                                        color: "#3D619B",
                            ...
                            <FormGroup sx={{ color: "#E9E9EB" }}>
                                <FormControlLabel control={<Checkbox defaultChecked checked={publicNoteChecked} onClick={onPublicNoteClickHandler}
                                ...
                                />} label="publiczna" />
                                <FormControlLabel control={<Checkbox checked={privateNoteChecked} onClick={onPrivateNoteClickHandler} sx={{
                                    color: "#E9E9EB",
                                    '&.Mui-checked': {
                                        color: '#EF4B4C',
                                    },
                            ...
                            <TextField label="hasło" sx={{ input: { color: 'black', bgcolor: "white", bordeColor: "white" } }} onChange={(e) => {
                            ...
                            <ListItem >
                                ...
                                >
                                    {note.passwordHash.length > 0 &&
                                        <Box sx={{ height: "50px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                            <Typography variant="h5" sx={{
                                                color: "black"
                                            }}>
                                            ...
                                            <Button
                                                onClick={() => { setIsInsertPasswordModalOpen(true) }}
                                            >
                                                Podaj hasło
                                            </Button>
                                        </Box>
                                    ...
                                    {note.passwordHash.length < 1 &&
                                        ...
                                        <Box sx={{ height: "50px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                            ...
                                            {note.userId === id && <Button
                                                onClick={() => { changeAccessibility(note.id) }}
                                            >
                                                Zmien na {note.isNotePublic ? "prywatną" : "publiczną"}
                                            </Button>}
                                        </Box>
                                        // <ReactMarkdown children={note.content}/>
                                    }
                ...
                <Box sx={{
                    ...
                    <Box sx={{
                        ...
                        <TextField
                            ...
                            onChange={(e) => { setNotePasswordToDecrypt(e.target.value) }}
                            sx={{
                                marginTop: "30px",
                                input: {
                                    color: 'black',
